...
export const BlockStream: React.FC<KineticStreamProps & { blockColor?: string }> = ({
  ...
  const blockWidth = interpolate(progress, [0, 0.5, 1], [0, 110, 0], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
      easing: Easing.bezier(0.8, 0, 0.2, 1)
  });
  
  const blockLeft = interpolate(progress, [0, 0.5, 1], [0, 0, 100], {
       extrapolateLeft: "clamp",
       extrapolateRight: "clamp",
       easing: Easing.bezier(0.8, 0, 0.2, 1)
  }); // Actually block grows from left, then shrinks to right usually?
  
  // Let's do: Block moves Left to Right.
  // Start: Left -100%, Width 100% (Offscreen left) -> No, block is specific to word size.
        ...
        {isTransitioning && prevGroup && (
          <div
            style={{
              ...
              // We want to rotate it "away"
              // rotateX(90deg) moves top towards viewer? No, rotateX(90deg) top goes back? 
              // Standard right-hand rule: Thumb right, fingers curl.
              // rotateX(90deg): Top goes AWAY (negative Z), Bottom comes TOWARDS.
              // We want "Flip Up". Top goes back.
...
export const TurbulenceStream: React.FC<KineticStreamProps> = ({
  ...
  // Distortion amount
  // 0 -> High -> 0
  const distortion = isTransitioning 
      ? interpolate(progress, [0, 0.5, 1], [0, 0.1, 0]) // 0.1 is actually quite strong for displacement scale
      : 0;
...
export const NeonStream: React.FC<KineticStreamProps & { neonColor?: string }> = ({
  ...
  if (Math.random() > 0.98) opacity = 0.2;
